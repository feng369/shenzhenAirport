...
public class SysRoleServiceImpl extends BaseServiceImpl<Sys_role> implements SysRoleService {
    ...
    @Aop(TransAop.READ_COMMITTED)
    public void linkUserAndRole(String roleCode, Sys_user user) {
        if (!Strings.isBlank(roleCode)) {
            Sys_role role = this.fetch(Cnd.where("code", "=", roleCode));
            if (role != null) {
                this.insert("sys_user_role", org.nutz.dao.Chain.make("roleId", role.getId()).add("userId", user.getId()));
            }
        }
    }
}
